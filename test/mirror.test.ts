...
describe('mirror', () => {
  it('runs expected git commands', () => {
    ...
    expect(execVerbose).toHaveBeenCalledWith('git fetch origin pull/42/head:pr-temp', '/tmp/mirror-repo');
    ...
    expect(execVerbose).toHaveBeenCalledWith('git commit --allow-empty -sS -m "chore: mirror pr-42"', '/tmp/mirror-repo');
    expect(execVerbose).toHaveBeenCalledWith('git push -u origin "mirror/pr-42"', '/tmp/mirror-repo');
  });
});
